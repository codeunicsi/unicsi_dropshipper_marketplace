...
export function useBulkOrdersAdmin() {
  ...
  const fetchOrders = useCallback(
    async (opts: {
      ...
      try {
        ...
        const msg =
          e instanceof Error ? e.message : "Failed to load bulk orders";
        setError(msg);
        setData(null);
        return null;
      } finally {
  ...
  const verifyPayment = useCallback(
    async (orderId: string, transactionReference?: string) => {
      const body =
  ...
  const rejectPayment = useCallback(async (orderId: string, reason: string) => {
    return apiClient.put(`${BASE}/${orderId}/reject-payment`, {
      reason: reason.trim(),
    });
  }, []);
