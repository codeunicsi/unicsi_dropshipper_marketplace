...
export default function BulkOrderConfigPage() {
  const { config, loading, error, fetchConfig, saveConfig, createConfig } =
    useBulkOrderConfig();
  const [form, setForm] = useState<BulkOrderConfigPayload>(
    defaultBulkOrderConfig,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ KEY LOGIC HERE
      if (!config) {
        // No config exists → CREATE (POST)
        await createConfig(form);
      } else {
        // Config exists → UPDATE (PUT)
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      <div className="border-b pb-6 mt-2 space-y-1">
        ...
        <p className="text-muted-foreground max-w-2xl">
          Step 1 of the manual B2B flow: who may place bulk orders, margin
          rules, status labels, settlement cycle, and the bank / UPI account
          buyers see when paying offline. Required before dropshippers can use
          bulk checkout APIs.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2 max-w-xs">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    defaultMarginPerPiece: Number(e.target.value) || 0,
                  }))
                }
              />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">
              Payment account (shown to buyers)
            </CardTitle>
          </CardHeader>
          <CardContent className="grid sm:grid-cols-2 gap-4">
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...
                    settlement: {
                      ...p.settlement,
                      cycle: e.target
                        .value as BulkOrderConfigPayload["settlement"]["cycle"],
                    },
                  }))
                }
              >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          >
            ...
            {/* ✅ Dynamic text */}
            {config ? "Edit Configuration" : "Save"}
          </Button>
        </div>
      </form>
    </div>
