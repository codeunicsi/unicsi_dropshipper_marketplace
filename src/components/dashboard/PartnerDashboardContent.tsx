...
function exportMasterCsv(params: {
  ...
  topProducts: { title: string; orderCount: number; units: number; gmv: number }[]
  bySupplier: { supplierName: string; orderCount: number; units: number; gmv: number }[]
  ...
  return (
    <div className="flex flex-col rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-1 text-xs font-medium text-zinc-500">
        {title}
        {showInfo ? <Info className="h-3.5 w-3.5 text-zinc-400" aria-hidden /> : null}
      </div>
      <p className="mt-2 text-2xl font-semibold tracking-tight text-zinc-900">{value}</p>
      {subPrimary ? (
        <p className={cn('mt-1 text-xs font-medium', subPrimaryClassName ?? 'text-zinc-500')}>
          {subPrimary}
        </p>
      ) : null}
      {footnote ? (
        <p className="mt-1 text-[11px] leading-snug text-zinc-400">{footnote}</p>
      ) : null}
    </div>
  )
}
...
export default function PartnerDashboardContent() {
  ...
  const shopifyRaw =
    (userRes as { data?: { shopify_store?: string | null } } | undefined)?.data?.shopify_store ??
    (profileRes as { data?: { shopify_store?: string | null } } | undefined)?.data?.shopify_store
  ...
  const [draftFrom, setDraftFrom] = useState(() => getPresetOrderAnalyticsRange(30).from)
  const [draftTo, setDraftTo] = useState(() => getPresetOrderAnalyticsRange(30).to)
  const [customError, setCustomError] = useState<string | null>(null)
  const [comparePrior, setComparePrior] = useState(false)
  const [mainTab, setMainTab] = useState<'overall' | 'product'>('overall')
  const [orderStatusFilter, setOrderStatusFilter] = useState<OrderStatusFilterBucket>('all')
  ...
  const appliedRange = useMemo(() => resolveOrderAnalyticsRange(rangeState), [rangeState])
  ...
  const applyCustomRange = () => {
    ...
    setCustomError(null)
    setRangeState({ kind: 'custom', from: draftFrom.trim(), to: draftTo.trim() })
  }
  ...
  const filteredByStatus = useMemo(
    () => byStatus.filter((r) => orderStatusInBucket(r.orderStatus, orderStatusFilter)),
  ...
  const productOptionsForPicker = useMemo(() => {
    ...
    return topProducts.filter((p) => p.title.toLowerCase().includes(productSearchNorm))
  ...
  const dominantPayment = useMemo(() => {
    ...
    const top = [...filteredByPaymentMethod].sort((a, b) => b.count - a.count)[0]
  ...
  const displayKpi = useMemo((): RoposoStyleKpis | null => {
    ...
    const rows = byStatus.filter((r) => orderStatusInBucket(r.orderStatus, orderStatusFilter))
  ...
  return (
    <div className="-mx-6 -mt-6 min-h-[calc(100vh-5rem)] bg-zinc-100/90 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-[1400px] space-y-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-zinc-600">
              Detailed insights for your bulk orders: GMV, platform margin share, delivery outcomes, and
              RTO — for the period you select (UTC).
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 shadow-sm">
              ...
              {shopHostname ? (
                <span className="max-w-[220px] truncate font-medium text-zinc-900" title={shopHostname}>
                  {shopHostname}
                </span>
              ) : (
                <Link
            ...
            <Button variant="outline" size="icon" className="border-zinc-200 bg-white shadow-sm" asChild>
        ...
        {mainTab === 'overall' ? (
          <>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-zinc-600">Order Status</Label>
                <Select
                  value={orderStatusFilter}
                  onValueChange={(v) => setOrderStatusFilter(v as OrderStatusFilterBucket)}
                >
                  ...
                  <SelectContent className="border-zinc-200 bg-white shadow-md" position="popper">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-zinc-600">Payment Method</Label>
                <Select
                  value={paymentMethodFilter}
                  onValueChange={(v) => setPaymentMethodFilter(v as PaymentMethodFilterBucket)}
                >
                  <SelectTrigger className="h-10 w-full border-zinc-200 bg-white font-semibold text-zinc-900 shadow-sm">
                    ...
                    <div className="max-h-60 overflow-y-auto p-1">
                      <button
                        ...
                        className={cn(
                          ...
                          productFilter === 'all' ? 'bg-zinc-100 font-medium' : 'hover:bg-zinc-50',
                        )}
                        onClick={() => {
            ...
            <p className="text-xs italic leading-relaxed text-zinc-500">
              Note: All metrics are based on platform bulk orders only. Date range denotes the order
              creation period (UTC).
            </p>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                {RANGE_OPTIONS.map((opt) => (
                  <Button
                    ...
                    className={cn(
                      ...
                      presetActive === opt.value && 'border-zinc-900 bg-zinc-900 text-white',
                    )}
                ...
                <div className="flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-2 py-1 shadow-sm">
                  <CalendarDays className="h-4 w-4 text-zinc-500" />
                  <input
                    type="date"
                    className="h-8 border-0 bg-transparent text-xs text-zinc-800 outline-none"
                    value={draftFrom}
                  ...
                  />
                  <Button type="button" size="sm" variant="secondary" className="h-7 text-xs" onClick={applyCustomRange}>
                    Apply
                  </Button>
                </div>
                <span className="text-xs text-zinc-500">{formatRangePill(range.from, range.to)}</span>
              </div>
              <div className="flex flex-wrap items-center gap-2">
            ...
            {displayKpi ? (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <KpiCard
                  ...
                  subPrimaryClassName={
                    displayKpi.inProgress > 0 ? 'text-amber-600' : 'text-zinc-500'
                  }
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <Card className="border-zinc-200 bg-white shadow-sm">
                <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                  <div>
                    <CardTitle className="text-base font-semibold text-zinc-900">Orders & GMV</CardTitle>
                    <CardDescription className="text-xs">Daily trend in this range</CardDescription>
                  </div>
                </CardHeader>
                <CardContent>
                  {loading && !chartRows.length ? (
                    <p className="py-20 text-center text-sm text-zinc-500">Loading chart…</p>
                  ) : (
                    <OrderTrendsChart
                      data={chartRows}
              ...
              <Card className="border-zinc-200 bg-white shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base font-semibold text-zinc-900">Platform margin %</CardTitle>
                  <CardDescription className="text-xs">As a share of GMV by day</CardDescription>
                </CardHeader>
                <CardContent>
                  {loading && !chartRows.length ? (
                    <p className="py-20 text-center text-sm text-zinc-500">Loading chart…</p>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <Card className="border-zinc-200 bg-white shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-semibold text-zinc-900">Orders by payment mode</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-zinc-200 bg-white shadow-sm">
                ...
                <CardContent>
                  ...
                  <p className="mt-2 text-xs text-zinc-500">Paid / proof / verified-like statuses</p>
                </CardContent>
              </Card>
              <Card className="border-zinc-200 bg-white shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-semibold text-zinc-900">Delivered orders</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-3xl font-semibold text-zinc-900">
                    {displayKpi?.deliveredCount ?? 0}
                  </p>
                  <p className="mt-2 text-xs text-zinc-500">In selected date range</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="border-zinc-200 bg-white shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-zinc-900">Orders by status</CardTitle>
                <CardDescription className="text-xs">Filtered view (order status filter above)</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filteredByStatus.map((row) => (
                          <tr key={row.orderStatus} className="border-b border-zinc-100">
                            <td className="px-3 py-2 text-zinc-900">{row.orderStatus}</td>
                            <td className="px-3 py-2 text-right tabular-nums text-zinc-800">{row.count}</td>
          ...
          <div className="space-y-4">
            <p className="text-sm text-zinc-600">
              Product-level performance for the same date range. Choose a product under{' '}
              <span className="font-medium">Product Name</span> on the Overall tab to focus one SKU.
            </p>
            <Card className="border-zinc-200 bg-white shadow-sm">
              <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
                <div>
                  <CardTitle className="text-base font-semibold text-zinc-900">Product analysis</CardTitle>
              ...
              <CardContent>
                {filteredTopProducts.length === 0 ? (
                  <p className="py-12 text-center text-sm text-zinc-500">No records found.</p>
                ) : (
                  <div className="overflow-x-auto rounded-md border border-zinc-200">
                      ...
                      <tbody>
                        {filteredTopProducts.map((row) => (
                          <tr key={row.productId} className="border-b border-zinc-100">
                            <td className="max-w-[240px] truncate px-3 py-2 text-zinc-900" title={row.title}>
                              {row.title}
                            </td>
                            ...
                            <td className="px-3 py-2 text-right tabular-nums">{formatInr(row.gmv)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <Card className="border-zinc-200 bg-white shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-zinc-900">By supplier</CardTitle>
                <CardDescription className="text-xs">Share of GMV by supplier</CardDescription>
              </CardHeader>
              <CardContent>
                {bySupplier.length === 0 ? (
                  <p className="text-sm text-zinc-500">No supplier breakdown.</p>
                ) : (
                  <div className="overflow-x-auto rounded-md border border-zinc-200">
                    <table className="w-full text-sm">
                      <thead>
                      ...
                      <tbody>
                        {bySupplier.map((row) => (
                          <tr key={row.supplierId} className="border-b border-zinc-100">
                            <td className="px-3 py-2 text-zinc-900">{row.supplierName}</td>
                            <td className="px-3 py-2 text-right tabular-nums">{row.orderCount}</td>
                            <td className="px-3 py-2 text-right tabular-nums">{row.units}</td>
                            <td className="px-3 py-2 text-right tabular-nums">{formatInr(row.gmv)}</td>
                          </tr>
        ...
        {loading ? (
          <p className="text-center text-xs text-zinc-500">Refreshing analytics…</p>
        ) : null}
      </div>
    </div>
  )
