...
export async function createCategory(body: {
  ...
  const res = (await apiClient.post(adminBase, body)) as {
    success: boolean;
    data: Category;
  };
...
export async function updateCategory(
  ...
  const res = (await apiClient.put(`${adminBase}/${id}`, body)) as {
    success: boolean;
    data: Category;
  };
...
export async function deactivateCategory(id: string): Promise<Category> {
  const res = (await apiClient.delete(`${adminBase}/${id}`)) as {
    success: boolean;
    data: Category;
  };
...
export async function deleteCategoryPermanent(id: string): Promise<void> {
  const res = (await apiClient.delete(`${adminBase}/${id}/permanent`)) as {
    success: boolean;
  };
